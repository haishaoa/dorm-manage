...
<script type="text/javascript">
    ...
    function dormBuildDelete(dormBuildId) {
        if (confirm("您确定要删除这个宿舍楼吗？")) {
            window.location = "DormBuildServlet?action=dormBuilddelete&dormBuildId=" + dormBuildId;
        }
    }
...
<div class="data_list">
    ...
    <form name="myForm" class="form-search" method="post" action="DormBuildServlet?action=dormBuildsearch">
        <button class="btn btn-success" type="button" style="margin-right: 50px;"
                onclick="javascript:window.location='DormBuildServlet?action=prepSave'">添加
        </button>
                ...
                <tr>
                    ...
                    <td>
                        <%--<button class="btn btn-mini btn-info" type="button"--%>
                                <%--onclick="javascript:window.location='DormBuildServlet?action=manager&dormBuildId=${dormBuild.dormBuildId }'">--%>
                            <%--管理员--%>
                        <%--</button>&nbsp;--%>
                        <button class="btn btn-mini btn-info" type="button"
                                onclick="javascript:window.location='DormBuildServlet?action=preSave&dormBuildId=${dormBuild.dormBuildId }'">
                            修改
                        </button>&nbsp;
