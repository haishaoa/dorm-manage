...
<script type="text/javascript">
	function checkForm(){
		...
		if(studentId==""||password==""||rPassword==""||name==""||sex==""||tel==""||dormBuildName==""||dormName==""){
...
<div class="data_list">
		...
		<form action="StudentServlet?action=studentadd" method="post" onsubmit="return checkForm()">
			<div class="data_form" >
				<%--<input type="hidden" id="studentId" name="studentId" value="${student.studentId }"/>--%>
					<table align="center">
						<tr>
							<td><font color="red">*</font>学号</td>
							<td><input style="margin-top:5px;height:30px;" type="text" id="studentId" name="studentId" value="${student.studentId }"/></td>
						...
						<tr>
							...
							<td>
								<select id="dormBuildName1" name="dormBuildName1" style="width: 90px;">
									<c:forEach var="dormBuild" items="${dormBuildList}">
										<option id="dormBuildName" value="${dormBuild.dormBuildId}" ${student.dormBuildId==dormBuild.dormBuildId?'selected':''}>${dormBuild.dormBuildName }</option>
									</c:forEach>
								</select>
							</td>
						</tr>
					...
					<div align="center">
						...
						&nbsp;<button class="btn btn-primary" type="button" onclick="javascript:window.location='StudentServlet?action=list'">返回</button>
					</div>
					<div align="center">
