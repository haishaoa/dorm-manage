...
<script type="text/javascript">
    ...
    function studentDelete(studentId) {
        if (confirm("您确定要删除这个学生吗？")) {
            window.location = "StudentServlet?action=studentdelete&studentId=" + studentId;
        }
    }
</script>
<style type="text/css">
...
<div class="data_list">
    ...
    <form name="myForm" class="form-search" method="post" action="StudentServlet?action=studentsearch" style="padding-bottom: 0px">
        <button class="btn btn-success" type="button" style="margin-right: 50px;"
                onclick="javascript:window.location='StudentServlet?action=preadd'">添加
        </button>
        <span class="data_search">
					...
					<option value="studentId" ${searchType eq "number"?'selected':'' }>学号</option>
					<option value="dormName" ${searchType eq "dorm"?'selected':'' }>寝室</option>
					</select>
                ...
                <tr>
                    ...
                    <td>
                        <button class="btn btn-mini btn-info" type="button"
                                onclick="javascript:window.location='StudentServlet?action=prechange&studentId=${student.studentId}'">
                            修改
                        </button>&nbsp;
